...
public class GetGoodsReceiptTypeService extends JobService {
        ...
        @SuppressLint("StaticFieldLeak") AsyncTask<Void, Void, Void> asyncTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    ...
                    if(helper.DeleteGoodsType()) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                            JSONObject jsonObject = jsonArray.getJSONObject(i);
                            gt.setiId(jsonObject.getString("iId"));
                            gt.setsName(jsonObject.getString("sName"));
                            boolean status = helper.InsertGoodsType(gt);
    ...
    public void PendingSO() {
        AndroidNetworking.get("http://"+new Tools().getIP(this)+ URLs.GetRemarksType)
